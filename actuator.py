import socket
import numpy as np
from ssl_simulation_robot_control_pb2 import RobotControl, MoveWheelVelocity, MoveGlobalVelocity, MoveLocalVelocity
...
class Actuator():
    ...
    def send_localVelocity_message(self, index, forward, left, angular, kick=0):
        '''
        Descrição:  
                Método responsável pelo envio da velocidade local do robô
                kick: 0 ou 1 (binário) para acionar chute
        '''
        self.robot_id = index
        ...
        if self.kick:
            robot_command.kick_speed = 1.0  # valor binário (1)
